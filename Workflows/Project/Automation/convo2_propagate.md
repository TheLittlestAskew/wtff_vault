...
CAMPAIGN STRUCTURE — TWO PARALLEL CAMPAIGNS: This vault is **Where the Flowers Forget (WtFF)**, DM Adam. **Where the Flowers Remember (WtFR)**, DM Alec, is a separate full campaign — propagate it ONLY as crossover, from the WtFF POV. WtFR characters already have **user-maintained roster cards** under `03-Characters/01 PCs/Where The Flowers Remember/`: cross-link crossover moments to those cards from WtFF pages, but **never create, overwrite, or expand them**, and never build WtFR lore in `04-World-Lore/`. The WtFF recruiter NPC is **Aiphrumfite "Aiph"** (`03-Characters/01 PCs/Aiphrumfite 'Aiph'.md`). See `_Campaign_Setup.md` → "Campaign structure".
